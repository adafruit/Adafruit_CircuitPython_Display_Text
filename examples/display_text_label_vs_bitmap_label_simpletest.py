...
for i, font_file in enumerate(font_files):
    ...
    if use_builtin_font:
        this_font=terminalio.FONT  # comment this out to switch back to BDF loaded fonts
    else:
...
preload_the_glyphs= True # set this to True if you want to preload the font glyphs into memory
...
bmap_label = label.Label(font=font_list[0], text=my_string, color=0xFFFFFF, max_glyphs=len(my_string),
